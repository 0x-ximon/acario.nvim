...
local DEFAULT_CONFIG = {
    ...
    colors = require("acario.palette_dark"),
    overrides = {},
    theme = "acario_dark",
}
...
local function get_configs()
    ...
    if g.colors_name == "acario_dark" then
        ...
        configs.colors = require("acario.palette_dark")
    elseif g.colors_name == "acario_light" then
        o.background = "light"
        configs.theme = "acario_light"
        configs.colors = require("acario.palette_light")
    end
...
---setup acario colorscheme
---@param configs AcarioConfig?
local function setup(configs)
    if type(configs) == "table" then
        user_configs = configs --[[@as AcarioConfig]]
    end
end

---load acario colorscheme
---@param theme string?
local function load(theme)
    if vim.fn.has("nvim-0.7") ~= 1 then
        vim.notify("acario.nvim: you must use neovim 0.7 or higher")
        return
    end
    ...
    g.colors_name = theme or "acario_dark"
